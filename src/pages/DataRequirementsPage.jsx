...
export default function DataRequirementsPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }} className="animate-fade-in-up">
      {/* Header */}
      <div>
        <h2 style={{ fontSize: 'var(--font-size-xl)' }}>데이터 요구사항 가이드</h2>
      ...
      <div className="grid-3">
        <div className="glass-card glow-cyan" style={{ padding: '20px', textAlign: 'center' }}>
          ...
          <h4 style={{ fontSize: '15px', color: '#f1f5f9' }}>사내 필수 연동 데이터</h4>
          <div style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--accent-cyan)', marginTop: '6px' }}>6개 데이터 소스</div>
          <span className="badge badge-cyan" style={{ fontSize: '9px', marginTop: '8px' }}>최우선 연동</span>
        </div>
        <div className="glass-card" style={{ padding: '20px', textAlign: 'center' }}>
          <div style={{ fontSize: '32px', marginBottom: '8px' }}>📋</div>
          <h4 style={{ fontSize: '15px', color: '#f1f5f9' }}>사내 선택 연동 데이터</h4>
          <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#fff', marginTop: '6px' }}>4개 데이터 소스</div>
          <span className="badge badge-purple" style={{ fontSize: '9px', marginTop: '8px', background: 'rgba(255,255,255,0.05)', color: '#94a3b8' }}>고도화 단계</span>
        </div>
        <div className="glass-card glow-purple" style={{ padding: '20px', textAlign: 'center' }}>
          <div style={{ fontSize: '32px', marginBottom: '8px' }}>🌐</div>
          <h4 style={{ fontSize: '15px', color: '#f1f5f9' }}>사외 외부 데이터</h4>
          <div style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--accent-purple)', marginTop: '6px' }}>3개 데이터 채널</div>
          <span className="badge badge-purple" style={{ fontSize: '9px', marginTop: '8px' }}>시장 벤치마크</span>
        </div>
      </div>

      {/* Category Tabs */}
      <div style={{ display: 'flex', borderBottom: '1px solid var(--border-subtle)', gap: '4px', overflowX: 'auto', paddingBottom: '1px' }}>
        {[
      ...
      <div className="glass-card animate-fade-in-up" style={{ padding: '24px' }}>
        {activeTab === 'essential' && (
          <div>
            <h3 className="card-title" style={{ marginBottom: '16px', fontSize: 'var(--font-size-base)' }}>🏢 사내 필수 연동 데이터 스펙</h3>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border-medium)', color: '#64748b' }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={index} style={{ borderBottom: '1px solid var(--border-subtle)', color: '#f1f5f9' }}>
                      <td style={{ padding: '12px', fontWeight: 'bold' }}>{row.name}</td>
                      <td style={{ padding: '12px', color: '#94a3b8' }}>{row.src}</td>
                      <td style={{ padding: '12px', fontFamily: 'monospace', color: '#a78bfa' }}>{row.format}</td>
                      <td style={{ padding: '12px', color: '#e2e8f0' }}>{row.usage}</td>
                      <td style={{ padding: '12px', fontWeight: 'bold', color: '#ef4444' }}>{row.priority}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'optional' && (
          <div>
            <h3 className="card-title" style={{ marginBottom: '16px', fontSize: 'var(--font-size-base)' }}>📋 사내 선택 연동 데이터 스펙 (고도화용)</h3>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border-medium)', color: '#64748b' }}>
                ...
                <tbody>
                  {[
                    { name: '리더십 다면진단', src: '설문 평가 시스템', format: '(사번, 리더십지표점수, 백분위 점수)', usage: '관리자/리더십 트랙 커리어 적합성 스코어 보강', priority: '🟡 선택 (권장)' },
                    ...
                    <tr key={index} style={{ borderBottom: '1px solid var(--border-subtle)', color: '#f1f5f9' }}>
                      ...
                      <td style={{ padding: '12px', fontFamily: 'monospace', color: '#a78bfa' }}>{row.format}</td>
                      <td style={{ padding: '12px', color: '#e2e8f0' }}>{row.usage}</td>
                      <td style={{ padding: '12px', fontWeight: 'bold', color: '#fbbf24' }}>{row.priority}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'external' && (
          <div>
            <h3 className="card-title" style={{ marginBottom: '16px', fontSize: 'var(--font-size-base)' }}>🌐 사외 채널 및 외부 데이터 연동</h3>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border-medium)', color: '#64748b' }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={index} style={{ borderBottom: '1px solid var(--border-subtle)', color: '#f1f5f9' }}>
                      <td style={{ padding: '12px', fontWeight: 'bold' }}>{row.name}</td>
                      <td style={{ padding: '12px', color: '#94a3b8' }}>{row.src}</td>
                      <td style={{ padding: '12px', color: '#a78bfa' }}>{row.format}</td>
                      <td style={{ padding: '12px', color: '#e2e8f0' }}>{row.usage}</td>
                      <td style={{ padding: '12px', fontWeight: 'bold', color: 'var(--accent-blue)' }}>{row.priority}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeTab === 'quality' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h3 className="card-title" style={{ fontSize: 'var(--font-size-base)' }}>📊 성공적 구축을 위한 데이터 품질 핵심 요건</h3>
            
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
              <div style={{ background: 'var(--bg-glass)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-subtle)' }}>
                <h4 style={{ fontSize: '13px', color: 'var(--accent-cyan)', marginBottom: '8px' }}>📊 통계적 임계점 (최소 표본수)</h4>
              ...
              <div style={{ background: 'var(--bg-glass)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-subtle)' }}>
                <h4 style={{ fontSize: '13px', color: 'var(--accent-cyan)', marginBottom: '8px' }}>🏷️ 표준 직무 분류체계 수립 선행</h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', lineHeight: '1.5' }}>
              ...
              <div style={{ background: 'var(--bg-glass)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-subtle)' }}>
                <h4 style={{ fontSize: '13px', color: 'var(--accent-cyan)', marginBottom: '8px' }}>🔄 실시간 동기화 주기 설계</h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', lineHeight: '1.5' }}>
              ...
              <div style={{ background: 'var(--bg-glass)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border-subtle)' }}>
                <h4 style={{ fontSize: '13px', color: 'var(--accent-cyan)', marginBottom: '8px' }}>✅ 결측치 보정 알고리즘</h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', lineHeight: '1.5' }}>
        ...
        {activeTab === 'privacy' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h3 className="card-title" style={{ fontSize: 'var(--font-size-base)' }}>🔒 개인정보 보호 및 AI 윤리 수칙</h3>
            <p style={{ fontSize: '12px', color: '#94a3b8', lineHeight: '1.5' }}>
              인사 데이터를 직원의 성장 목적으로 사용할 때는 프라이버시 보호와 편향 제거가 극도로 중요합니다. 실제 설계서에 포함되어야 하는 5가지 윤리 기준입니다.
            </p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '8px' }}>
              <div style={{ background: 'rgba(167, 139, 250, 0.03)', border: '1px solid rgba(167, 139, 250, 0.1)', borderRadius: '8px', padding: '14px' }}>
                <h4 style={{ fontSize: '13px', color: '#c4b5fd', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>🔒</span> 1. 개인 식별 데이터(PII) 난수화 및 익명 처리
                </h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', paddingLeft: '22px' }}>
              ...
              <div style={{ background: 'rgba(167, 139, 250, 0.03)', border: '1px solid rgba(167, 139, 250, 0.1)', borderRadius: '8px', padding: '14px' }}>
                <h4 style={{ fontSize: '13px', color: '#c4b5fd', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>👥</span> 2. 코호트 비교 모듈의 임계값 가드 (k-익명성)
                </h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', paddingLeft: '22px' }}>
              ...
              <div style={{ background: 'rgba(167, 139, 250, 0.03)', border: '1px solid rgba(167, 139, 250, 0.1)', borderRadius: '8px', padding: '14px' }}>
                <h4 style={{ fontSize: '13px', color: '#c4b5fd', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>🚫</span> 3. 구성원의 정보 주권 보장 (Opt-out 권리)
                </h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', paddingLeft: '22px' }}>
              ...
              <div style={{ background: 'rgba(167, 139, 250, 0.03)', border: '1px solid rgba(167, 139, 250, 0.1)', borderRadius: '8px', padding: '14px' }}>
                <h4 style={{ fontSize: '13px', color: '#c4b5fd', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>📋</span> 4. 설명 가능한 AI 추천 근거 투명성 (Explainable AI)
                </h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', paddingLeft: '22px' }}>
              ...
              <div style={{ background: 'rgba(167, 139, 250, 0.03)', border: '1px solid rgba(167, 139, 250, 0.1)', borderRadius: '8px', padding: '14px' }}>
                <h4 style={{ fontSize: '13px', color: '#c4b5fd', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>🔍</span> 5. 추천 알고리즘의 편향성 지속 정기 감사
                </h4>
                <p style={{ fontSize: '11px', color: '#94a3b8', paddingLeft: '22px' }}>
